  ...
  return (
    <div className={`rounded-2xl border transition-all duration-300 ${open ? 'border-slate-200/60 bg-white/20' : 'border-transparent bg-white/10'}`}>
      <button onClick={() => setOpen(!open)} className="w-full flex items-center gap-3 px-4 py-3 text-right cursor-pointer">
        ...
        <span className="text-base lg:text-lg font-bold text-slate-700 flex-1">{title}</span>
        <svg className={`w-5 h-5 text-slate-400 transition-transform duration-300 ${open ? 'rotate-180' : ''}`}
  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-2">
        <label className="text-sm lg:text-base text-slate-600 font-medium flex-shrink-0">{label}</label>
        <div className="flex items-center gap-1 bg-white/60 rounded-lg px-2.5 py-1.5 border border-white/70">
          <input type="number" value={value ?? 0} onChange={e => onChange(Number(e.target.value) || 0)} step={step}
            className="w-28 text-base lg:text-lg font-semibold text-slate-800 bg-transparent outline-none text-left" />
          <span className="text-xs lg:text-sm text-slate-400">{suffix}</span>
        </div>
      </div>
      {rec && <p className="text-xs lg:text-sm text-indigo-500 mt-1 mr-0.5">💡 המלצה: {rec}</p>}
      {note && <p className="text-xs lg:text-sm text-slate-400 mt-0.5 mr-0.5">{note}</p>}
    </div>
  );
}
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1.5">
        <label className="text-sm lg:text-base text-slate-600 font-medium">{label}</label>
        <span className="text-base lg:text-lg font-bold text-indigo-600 num">{displayValue}</span>
      </div>
      <input type="range" value={value} onChange={e => onChange(Number(e.target.value))}
        min={min} max={max} step={step}
        className="w-full h-2 bg-slate-200 rounded-full appearance-none cursor-pointer accent-indigo-600" />
      {rec && <p className="text-xs lg:text-sm text-indigo-500 mt-1">💡 המלצה: {rec}</p>}
      {note && <p className="text-xs lg:text-sm text-slate-400 mt-0.5">{note}</p>}
    </div>
  );
}
...
export function InputPanel({ config, setConfig }: Props) {
  ...
  // House purchase slider: value 0 = never, 1-30 = years from start
  ...
  return (
    <div className="glass-card p-5 lg:p-6 text-right lg:sticky lg:top-4 lg:max-h-[calc(100vh-2rem)] lg:overflow-y-auto space-y-3">
      <div className="flex items-center gap-3 pb-3 border-b border-slate-200/40">
        <img src={`${import.meta.env.BASE_URL}profile.jpeg`} alt="" className="w-12 h-12 lg:w-14 lg:h-14 rounded-xl object-cover shadow" />
        <div>
          <h2 className="text-lg lg:text-xl font-bold text-slate-800">פרמטרים</h2>
          <p className="text-xs lg:text-sm text-slate-400">יותם והדס · כל השדות ניתנים לעריכה</p>
        </div>
      </div>
      ...
      {/* ═══ קניית בית ═══ */}
      <Section title="קניית בית" icon="🏠" defaultOpen={true}>
        <SliderInput label="שנת קניית בית"
        ...
        {config.housePurchaseYear !== null && (
          <>
            ...
            <NumInput label="ריבית" value={Math.round(config.house.mortgageRate * 1000) / 10}
              ...
              rec="4.5-5.5% — ריבית ממוצעת משוקללת" />
      ...
      <Section title="נכסים משותפים" icon="💰">
        ...
        <NumInput label="תמורת דירה (נטו)" value={config.assets.apartmentNetProceeds}
          ...
          rec="1,180,000 ₪ — אור עקיבא, פטור ממס שבח" />
      </Section>
      ...
      <Section title="הנחות שוק" icon="📈">
        ...
        <SliderInput label="אינפלציה" value={Math.round(config.market.inflationRate * 100 * 10) / 10}
          ...
          rec="2.5% — יעד בנק ישראל, בפועל 2.0% (פב׳ 2026)" />
